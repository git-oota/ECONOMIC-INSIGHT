...
def generate_content():
    ...
    【構成（contents内）】
    1. 事実：今何が起きているか（高市首相の解散検討など）。
    2. 解説：なぜ解散するのか、誰が得するのか（ゲームのリセット等の比喩を使って）。
    3. 分析：円安と金利上昇が「私達」の生活にどう響くか。

    【重要ルール】
    - 出力は必ず以下のJSON形式のみとし、他の文章は一切含めないでください。
    - keys: "id", "date", "titles", "contents", "mermaid", "glossary"
    - titles, contents, mermaid, glossary は、それぞれ "ja" と "en" を持つこと。
    - タイムアウトを避けるため、検索は最小限にし、全体を1000文字程度で素早く執筆してください。
    """
